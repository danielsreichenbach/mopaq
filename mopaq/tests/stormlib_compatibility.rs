...
use mopaq::{archive::Archive, builder::ArchiveBuilder, header::FormatVersion};
use std::fs;
...
fn test_multi_compression_format() {
    ...
    let builder = ArchiveBuilder::new()
        ...
        .add_file_data_with_options(
            test_data.clone(),
            "audio.wav",
            mopaq::compression::flags::ADPCM_MONO | mopaq::compression::flags::ZLIB,
            false, // encrypt
            0,     // locale
        );
...
fn test_uncompressed_small_data() {
    ...
    let builder = ArchiveBuilder::new()
        ...
        .add_file_data_with_options(
            test_data.to_vec(),
            "small.txt",
            mopaq::compression::flags::ZLIB, // Request compression
            false,                           // encrypt
            0,                               // locale
        );
